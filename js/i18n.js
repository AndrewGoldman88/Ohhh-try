...
function applyLang() {
    ...
    const qRefresh = document.querySelector('.quote-refresh');
    if (qRefresh) qRefresh.textContent = L('quote_refresh');
    ...
    document.querySelectorAll('#view-stats .section-title').forEach((el, i) => {
        // Порядок: Statistics, Streak history, Mood, Relapses, Sex (скрытый), Trigger map
        const titles = [
            L('stats_title'), L('stat_streak_history'), L('stat_mood'),
